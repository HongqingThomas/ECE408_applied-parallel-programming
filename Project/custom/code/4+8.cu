...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    extern __shared__ float SM_load[];
    ...
    #define sm3d_tree(i2, i1, i0) SM_load[i2 * TILE_WIDTH * Channel + i1 * Channel + i0]
    ...
    if (h_out < Height_out && w_out < Width_out)
    {
        ...
        for (int p = 0; p < K; p++)
        {
            for (int q = 0; q < K; q++)
            {
                result += in_4d(b,c, h_out + p, w_out + q) * mask_4d(m, c, p, q);
            }
        }
        
        sm3d_tree(threadIdx.y, threadIdx.x, c) = result;

        // tree reduction
        for (int stride = 1; stride < Channel; stride *= 2)
        {
            __syncthreads();
            if ((c % (2 * stride) == 0) && (c + stride < Channel))
            {
                sm3d_tree(threadIdx.y, threadIdx.x, c) += sm3d_tree(threadIdx.y, threadIdx.x, c + stride);
            }
        }
        __syncthreads();
        if (c == 0)
        {
            out_4d(b, m, h_out, w_out) = sm3d_tree(threadIdx.y, threadIdx.x, 0);
        }
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // dim3 DimBlock(TILE_WIDTH,TILE_WIDTH,K); // can not use this one, maybe the reason is that K is too large for threadDim.z's size
